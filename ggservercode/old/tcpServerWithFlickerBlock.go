...
func getInfoForPlayer(playerId int) string {
	var message string
	for index, currentObject := range blockList {
		distanceX := getDifference(playerList[playerId].x, currentObject.x)
		distanceY := getDifference(playerList[playerId].y, currentObject.y)
		...
		if lineDistance <= playerList[playerId].renderDistance {
			fmt.Println("Visible")
			message = message + " { x: " + strconv.Itoa(currentObject.x) + ", y: " + strconv.Itoa(currentObject.y) + " }"
		}
		index = index
...
func main() {
	blockList = append(blockList, block{x: 4, y: 4, height: 1, width: 1})
	blockList = append(blockList, block{x: 6, y: 4, height: 0, width: 0, flicker: true})
        PORT := ":9876"
